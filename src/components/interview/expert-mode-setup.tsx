...
interface ExpertModeSetupProps {
...
export function ExpertModeSetup({ onStart, onBack }: ExpertModeSetupProps) {
  ...
  return (
    <div className="space-y-8" data-testid="expert-mode-setup">
      {/* Back button */}
      <button
        type="button"
        onClick={onBack}
            ...
            >
              <p
                className={`text-sm font-medium ${
                  questionMode === mode.id ? "text-amber-700" : "text-slate-700"
                }`}
              >
                {mode.label}
              </p>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
          {QUESTION_TYPES.map((qt) => (
            ...
            >
              ...
              <span
                className={`text-sm font-medium ${
                  selectedTypes.includes(qt.id) ? "text-amber-700" : "text-slate-700"
                }`}
              >
                {qt.label}
              </span>
